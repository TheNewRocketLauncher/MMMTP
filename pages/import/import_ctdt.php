...
if ($mform2->is_cancelled()) {
    ...
    if($fromform->link != null){
        ...
        if (($handle = fopen( $fromform->link, "r")) !== FALSE) {
            ...
            while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                ...
                if($data[0] == 1.3 || $data[0] == '1.3'){
                    $arr_chuandaura_ctdt[] = ['ma_cay_cdr' => $data[1], 'ma_cdr' => $data[2], 'ten' => $data[3], 'mota' => $data[4], 'level_cdr' => $data[5]];
                }else
        ...
        if($is_check_ctdt_hople){
                ...
                if ($check == false) {
                    ...
                    if ($check2 == false) {
                        if ($item['ma_cay_cdr'] == 'ma_cay_cdr' || $item['level_cdr'] == 'level_cdr') {
                            ...
                            $i_chuandaura[] = ['ma_cay_cdr'=>$item['ma_cay_cdr'], 'ma_cdr'=>$item['ma_cdr'],
            ...
            foreach($arr_caykhoikienthuc as $iarr_caykhoikienthuc){
                ...
                $param = new stdClass();
                $param->ma_cay_khoikienthuc = $iarr_caykhoikienthuc['ma_caykhoikienthuc'];
                $param->ma_tt = $iarr_caykhoikienthuc['ma_tt'];
                $param->ma_khoi =  $iarr_caykhoikienthuc['ma_khoi'];
                $param->ma_khoicha = $iarr_caykhoikienthuc['ma_khoicha'];
                $param->ten_cay = $iarr_caykhoikienthuc['ten_cay'];
                $param->mota = $iarr_caykhoikienthuc['mota'];

                // insert('eb_cay_khoikienthuc', $param);
            }
            ...
            $param_ctdt->ma_cay_khoikienthuc = $arr_caykhoikienthuc[1]['ma_caykhoikienthuc'];
    ...
    if($link != null){
            ...
            foreach($i_chuandaura as $ii_chuandaura){
                ...
                if($ii_chuandaura['level_cdr'] == 1 || $ii_chuandaura['level_cdr'] == '1'){

                    echo "<h4 ><strong style='margin-left: 70px;font-weight: 600; font-family: auto;'> ❖ " . $ii_chuandaura['ten'] ."</strong></h4>" ; 

                    $arr_level2 =  find_chuandaura_con($i_chuandaura,  $ii_chuandaura['ma_cdr'], 2, $ii_chuandaura['ma_cay_cdr']);

                    
                    $arr_level3 = array();
                    foreach($arr_level2 as $iarr_level2){

                        echo"<h4 style='margin-left: 7% ; margin-bottom: 0; font-family: auto;'> - ". $iarr_level2['ten'] . "</h4>"; echo "<br>";

                        $arr_level3 = find_chuandaura_con($i_chuandaura,  $iarr_level2['ma_cdr'], 3, $ii_chuandaura['ma_cay_cdr']);

                        foreach($arr_level3 as $iarr_level3){

                            echo "<h4 style='margin-left: 10%; margin-bottom: 0; font-family: auto;'> • " . $iarr_level3['ten'] . "</h4>"; echo "<br>";
                        }
                            ...
                            if($ilist_monhoc['monbb']){
                                foreach($ilist_monhoc['monbb'] as $mamonhoc){
                                    ...
                                    if($chitiet_monhoc){
                                        ...
                                        $table->data[] = [$idx, $chitiet_monhoc['mamonhoc'], $chitiet_monhoc['tenmonhoc'], $chitiet_monhoc['sotc'],
                                ...
                                foreach($ilist_monhoc['montc'] as $tc_monhoc){
                                    ...
                                    foreach($tc_monhoc['listmon'] as $mamonhoc){
                                        ...
                                        if($chitiet_monhoc){
                                            ...
                                            $table->data[] = ['', $chitiet_monhoc['mamonhoc'], $chitiet_monhoc['tenmonhoc'], $chitiet_monhoc['sotc'],
                                                $chitiet_monhoc['sotiet_lt'], $chitiet_monhoc['sotiet_th'], $chitiet_monhoc['sotiet_bt'], 
                                                $chitiet_monhoc['loaihocphan'], $chitiet_monhoc['ghichu']];
...
function tonghop_cay($arr_khoikienthuc, $arr_caykhoikienthuc){
    ...
    foreach($arr_caykhoikienthuc as $iarr_caykhoikienthuc){
        ...
        if( count($level_cay) == 1){ // dai cuong va hoi
            ...
            $arr_tam[] = ['ma_tt'=>$iarr_caykhoikienthuc['ma_tt'], 'ma_khoi'=>$iarr_caykhoikienthuc['ma_khoi'],
            ...
            foreach($node_cons as $inode_cons){
                ...
                $arr_tam[] = ['ma_tt'=>$inode_cons['ma_tt'], 'ma_khoi'=>$inode_cons['ma_khoi'],
...
function get_errors_monhoc($arr_khoikienthuc, $arr_error_khoi ,$arr_caykhoikienthuc, $arr_monhoc){
    ...
    $arr_error = array();$arr_khoi_availble = array();
...
function find_node_con($arr_caykhoikienthuc, $ma_tt){
    ...
    foreach($arr_caykhoikienthuc as $iarr_caykhoikienthuc){
        ...
        // echo startsWith($iarr_caykhoikienthuc['ma_tt'], $ma_tt); echo '<br>';
...
function get_root_node($arr_caykhoikienthuc){
    ...
    return current(array_filter($arr_caykhoikienthuc, function($e) { return $e['ma_tt']==0 || $e['ma_tt']== ''; }));
...
function find_mininode($arr_tem1, $node_root){
    $arr = array();
    echo 'amkhoi'.$node_root['ma_khoi']; echo '<br>';

    foreach($arr_tem1 as $iarr_tem1){
        echo $iarr_tem1['ma_khoi'] . '- ' .$iarr_tem1['ma_khoicha']; echo '<br>';
