...
if ($mform2->is_cancelled()) {
    ...
    $is_check_monhoc_hople = check_monhoc_hople($mamonhoc, $ma_ctdt);
    $is_check_ctdt_hople = check_ctdt_hople($mamonhoc, $ma_ctdt);
    ...
    if(!$is_check_monhoc_hople){
        echo 'Mã môn học <strong>'.$mamonhoc.  '</strong> không hợp lệ hoặc chưa tồn tại';
        
    }echo "<br>";
    if(!$is_check_ctdt_hople){
        echo 'Mã chương trình đào tạo <strong>'  .$ma_ctdt.  '</strong> không hợp lệ hoặc không chứa môn học <strong>'.$mamonhoc . '</strong>';
    }echo "<br>";
    
    if($is_check_decuong_hople && $is_check_monhoc_hople && $is_check_ctdt_hople){
        ...
        foreach($arr_kh_giangday_lt as $iarr_kh_giangday_lt){
                ...
                $check_result =  check_chuandaura_hople($param_kh_giangday_lt->danhsach_cdr);
        ...
        foreach($arr_danhgiamonhoc as $iarr_danhgiamonhoc){
                ...
                $check_result =  check_chuandaura_hople($param_danhgia->chuandaura_danhgia);
    ...
    if($link != null){
        ...
        $is_check_monhoc_hople = check_monhoc_hople($arr_thongtin[0]['mamonhoc'], $ma_ctdt);
        $is_check_ctdt_hople = check_ctdt_hople($arr_thongtin[0]['mamonhoc'], $arr_thongtin[0]['ma_ctdt']);
        
        echo "<strong style='text-decoration: underline;'>LOG</strong>"; 
        if(!$is_check_decuong_hople){
            echo "Mã đề cương <strong style='color: red'>".$arr_thongtin[0]['ma_decuong']. "</strong> không hợp lệ hoặc đã tồn tại"; 
            
        }echo "<br>";
        if(!$is_check_monhoc_hople){
            echo "Mã môn học <strong style='color: red'>".$arr_thongtin[0]['mamonhoc'].  "</strong> không hợp lệ hoặc chưa tồn tại";
            
        }echo "<br>";
        if(!$is_check_ctdt_hople){
            echo "Mã chương trình đào tạo <strong style='color: red'>"  . $arr_thongtin[0]['ma_ctdt'].  "</strong> không hợp lệ hoặc không chứa môn học <strong style='color: red'>".$arr_thongtin[0]['mamonhoc'] . "</strong>";
        }echo "<br>";echo "<br>";   
...
function check_chuandaura_hople($item_chuandaura){
