...
if ($mform->is_cancelled()) {
    ...
    if (validateData($fromform)) {
        ...
        if(get_ctdt_byMa($param->ma_ctdt)){
            $param->ma_ctdt .= "1";
        }
        
        while(get_ctdt_byMa($param->ma_ctdt)){
            $param->ma_ctdt++;
        }

        $id = insert_ctdt($param, $chuandaura);
        echo '<h2>Thêm mới thành công!</h2>';
        echo '<br>';
        
        $url = new \moodle_url('/blocks/educationpgrs/pages/ctdt/chitiet_ctdt.php', ['id' => $id]);
        $linktext = 'Chi tiết chương trình đào tạo';
...
//     $formdata->tennganh = '';
//     $formdata->tenhe = '';
//     $formdata->tennienkhoa = '';
//     $formdata->tenchuyennganh = '';
