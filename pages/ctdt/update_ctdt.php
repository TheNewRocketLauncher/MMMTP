...
$mform = new edit_ctdt_form();

// Form processing
if ($mform->is_cancelled()) {
    // Button cancel    
} else if ($mform->no_submit_button_pressed()) {
    echo $mform->get_submit_value($id_ctdt);
    echo 'no_submit_button_pressed';
} else if ($fromform = $mform->get_data()) {
    
    echo $fromform->id_ctdt;
    if(can_edit_ctdt($fromform->id_ctdt)){
        if (validateData($fromform)) {
            echo $fromform->id_ctdt;

            // $param = new stdClass();
            
            // $param->id = $fromform->id;
            ...
            // update_ctdt($param, $fromform->select_cdr);
    ...
    if($id != NULL){
        ...
        $list_cdr = get_list_cdr_thuoc_ctdt($ctdt->ma_ctdt);
        $arr_cdr = array();
        ...
        echo $ctdt->id;
        // echo json_encode($list_cdr);
    
        $formdata = new stdClass();
        $formdata->key_ctdt = $id;
    
        $formdata->txt_ma_ctdt = $ctdt->ma_ctdt;
        $formdata->txt_tct = $ctdt->mota;
        $formdata->mabac = $ctdt->ma_bac;
        $formdata->bacdt = $info_ctdt->ma_bac;
    
        $formdata->editor_muctieudt_chung['text'] = $ctdt->muctieu_daotao;
        $formdata->editor_muctieudt_cuthe['text'] = $ctdt->muctieu_cuthe;

        $formdata->select_cdr = $arr_cdr;

        $formdata->editor_muctieudt_chnn['text'] = $ctdt->cohoi_nghenghiep;
        $formdata->txt_tgdt = $ctdt->thoigian_daotao;
        $formdata->txt_klkt = $ctdt->khoiluong_kienthuc;
        $formdata->txt_dtts = $ctdt->doituong_tuyensinh;

        $formdata->editor_qtdt['text'] = $ctdt->quytrinh_daotao;
        $formdata->editor_dktn['text'] = $ctdt->dieukien_totnghiep;
        $formdata->select_caykkt = $ctdt->ma_cay_khoikienthuc;
...
{
    if($data->select_cdr == NULL){
        echo '<h3>Cần chọn ít nhất một chuẩn đầu ra cho chương trình đào tạo<h3>';
        return false;
    }
    if(exist_ma_ctdt($data->txt_ma_ctdt)){
        echo '<h3>Mã Chương trình đào tạo đã tồn tại<h3>';
    }
